    ...
    for (let i = 0; i < resources.length; i++) {
        ...
        if (
            ...
            try {
                ...
                core.error(ex instanceof Error ? ex : String(ex));
...
export async function checkPodStatus(
    ...
    for (let i = 0; i < iterations; i++) {
        ...
        if (namespace) {
            ...
            console.trace(`namespace not defined for pod: ${podName}`);
            podStatus = await getPodStatus(kubectl, {
                name: podName,
                type: "pod",
            });
        ...
        if (podStatus && podStatus?.phase !== "Pending" && podStatus?.phase !== "Unknown") {
            break;
        }
    }
...
function isLoadBalancerIPAssigned(status: IngressStatus) {
    return (status.loadBalancer?.ingress?.length ?? 0) > 0;
}
