...
describe("Kubernetes types", () => {
    ...
    it("contains deployment types", () => {
        const expected = ["deployment", "replicaset", "daemonset", "pod", "statefulset"];
        expect(expected.every((val) => DEPLOYMENT_TYPES.includes(val as any))).toBe(true);
    });

    it("contains workload types", () => {
        const expected = ["deployment", "replicaset", "daemonset", "pod", "statefulset", "job", "cronjob"];
    ...
    it("contains workload types with rollout status", () => {
        ...
        expect(expected.every((val) => WORKLOAD_TYPES_WITH_ROLLOUT_STATUS.includes(val as any))).toBe(true);
    });

    describe("isDeploymentEntity", () => {
        it("checks if kind is undefined", () => {
            // throws on no kind
            expect(() => isDeploymentEntity({ kind: undefined } as any)).toThrow(ResourceKindNotDefinedError);
        });
    ...
    describe("isWorkloadEntity", () => {
        it("checks if kind is undefined", () => {
            // throws on no kind
            expect(() => isWorkloadEntity("")).toThrow(ResourceKindNotDefinedError);
        });
    ...
    describe("isServiceEntity", () => {
        it("throws on no kind", () => {
            expect(() => isServiceEntity({ kind: undefined } as any)).toThrow(ResourceKindNotDefinedError);
        });
    ...
    it("checks if kind is ingress entity", () => {
        // throws on no kind
        expect(() => isIngressEntity({ kind: undefined } as any)).toThrow(ResourceKindNotDefinedError);
