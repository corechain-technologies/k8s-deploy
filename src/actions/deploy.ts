...
export async function deploy(
    ...
    const trafficSplitMethodString = core.getInput("traffic-split-method", { required: true });
    const trafficSplitMethod = parseTrafficSplitMethod(trafficSplitMethodString);

    if (!trafficSplitMethod) {
        throw new Error(`Invalid traffic split method: ${trafficSplitMethod}`);
    }
